...
public class Layer {
    ...
    public double getLayerError() { return layerError; }
    ...
    public void optimize (double networkError, double learningRate, int iteration) { // Optimize all layer neurons.
        this.setLayerError(networkError, learningRate, iteration);
        for (Neuron neuron : this.neurons) { neuron.optimize(this.layerError, learningRate, iteration); }
    }

    private void setLayerError(double NetworkError, double learningRate, int iteration) {
        double avgHypothesis, sum = 0;
        for (Neuron neuron : this.neurons) { sum += neuron.getHypothesis(); }
        avgHypothesis = sum / this.neurons.length;
        this.layerError = (((this.layerError * (iteration - 1)) + (avgHypothesis / NetworkError)) / iteration) * learningRate;
    }
