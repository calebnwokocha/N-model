...
public class linear_regression_with_complex_numbers {
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 1; i < trainSets.length; i++){
            for (int j = 0; j < trainSets[i].length; j++) {
                trainSets[i][j][0] = (Math.random() + i) * -1;
            }
        }

        for (int i = 1; i < testSets.length; i++){
            for (int j = 0; j < testSets[i].length; j++) {
                testSets[i][j][0] = (Math.random() + i) * -1;
            }
        }

        Function<Double[], Double> cube = x -> Math.pow(x[0], 3.0);
        Function<Double[], Double> square = x -> Math.pow(x[0], 2.0);

        for (int i = 1; i < networks.length; i++){
            networks[i] = new Network(5, 1);
            if (i > 4) { networks[i].setCFunction("sum", 2.0, square); }
            else { networks[i].setCFunction("sum", 3.0, cube); }
